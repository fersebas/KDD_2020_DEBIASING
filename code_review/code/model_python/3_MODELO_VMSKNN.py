...
# # Cargamos librerías y función principal del VMSKNN
...
#from pympler import asizeof # Comentar para que funcione 
import numpy as np
import pandas as pd
...
class VMContextKNN:
    '''
    VMContextKNN( k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div', 
    ...
    weighting : string
        Decay function to determine the importance/weight of individual actions in the current session 
        (linear, same, div, log, quadratic). (default: div)
    weighting_score : string
        Decay function to lower the score of candidate items from a neighboring sessions that were selected by less recently 
        clicked items in the current session. (linear, same, div, log, quadratic). (default: div_score)
...
# DA ERROR OR NO ENCONTRAR EL ARCHIVO: ¿Añadir esto????

#items_feat = pd.read_csv("Underexpose_item_feat_prin_all.txt", header=None).rename(columns={0: "item_id"})
...
recom_vali = pd.DataFrame()          # El dataframe donde guardamos todas las recomendaciones de validacion. 
...
#c = 0
    ...
    click_valid =  click_test.groupby("user_id").tail(1)         # De cada usuario de test, extraemos la fila de su último click

    # Lo quitamos de test
    click_test = click_test.drop(click_valid.index).reset_index(drop=True) # Quitamos los últimos clicks de test (futurazos) 
    click_valid.reset_index(drop=True,inplace=True)                        # Reordenamos índices

    # tambien quitamos el item_id de los whole_click
    rem = pd.merge(whole_click, click_valid, how="left",on=["user_id","item_id"]) # Solo tienen time no NaN users de click_valid
    whole_click_valid = rem[rem["time"].isna()].reset_index(drop=True)            # Se queda solo las observaciones con time NaN
    # Al final lo que se hace aquí es que las observaciones de validación se quitan de wholeclick

    # Juntamos clicks de train y test 
    all_click = click_train.append(click_test)  
    # nos aseguramos que no haya duplicados y ordenamos por time, pa que cuando se filtre por user esté filtrado por usuario y time
    all_click = all_click.drop_duplicates(subset=['user_id','item_id','time'],keep='last')
    all_click = all_click.sort_values('time')
    all_click= all_click.sort_values(["user_id","time"]).reset_index(drop=True)      # Reorden de índices

    
    model = VMContextKNN(5000,10000, session_key = 'user_id', item_key= 'item_id', time_key= 'time',
                    similarity = "cosine", weighting = "quadratic", weighting_score="div", idf_weighting=5, normalize=True)
    model.fit(all_click)

    recom_item_valid = []    # Lista que completaremos en cada paso

    
    # SACAMOS LOS PENÚLTIMOS CLICKS DE CADA USUARIO DE TEST, QUE USAREMOS PARA PREDECIR: 
    penultimosclicks = click_test.groupby('user_id').tail(1)      # Es el tail(1) porque ya le hemos quitado el último antes  

    objetos = all_click.item_id.unique()    #  Creamos un array con el listado de todos los items distintos que hay 

    for users in tqdm(click_test['user_id'].unique()):     # Los usuarios de click_test son los mismos que los de click_valid   
    
            penultimo_click =  int(penultimosclicks['item_id'][penultimosclicks['user_id']==users].values)
            
            preds = model.predict_next(users, penultimo_click, objetos).sort_values(ascending=False)  # lista con el item y su peso
    
            historial = all_click[all_click['user_id']==users].item_id.unique()    # todos los items clicados por el usuario
            preds_index = preds.index
            preds_index_filter = preds_index[~preds_index.isin(historial)]         # Eliminamos de la recomendacion los ya clickados
            for j in preds_index_filter[0:NUM_RECOM]: # DAMOS 500 RECOMENDACIONES
                recom_item_valid.append([users, j, preds[j]])  
                
    recom_df = pd.DataFrame(recom_item_valid, columns=['user_id', 'item_id',"sim"])  # De la lista anterior, creamos un dataframe
    recom_vali = recom_vali.append(pd.DataFrame(recom_item_valid))
    

    # ---------------- 4 LÍNEAS PARA QUITAR LOS ITEMS ALEATORIOS DE SIMILARIDAD 0 (quitar las 2 de arriba) --------------------
    #recom_fase = pd.DataFrame(recom_item_valid, columns=['user_id', 'item_id', 'sim_VMSKNN']) # Hacemos df la reco de la fase
    #recom_fase = recom_fase[recom_fase["sim_VMSKNN"]>0] # Eliminamos items de similaridad 0 (aleatorios)
    #recom_df = recom_fase
    #recom_vali = recom_vali.append(recom_fase) 
    # -------------------------------------------------------------------------------------------------------------------------
    
    
    # CON ESTO LE DAMOS LA SALIDA DE: USUARIO --> LISTADO DE 500 ITEMS
    result = recom_df.groupby('user_id')['item_id'].apply(lambda x: ','.join([str(i) for i in x])).str.split(',', expand=True).reset_index()  

    # Le pegamos el target y calculamos el ncdg y hitrate full(bsta saber la posición que ocupa el target en la matriz de recomend.)
    result["target"]= click_valid["item_id"]
...
recom_vali.head(2) # Guarda todas las mejores recomendaciones de los usuarios de todas las fases


# In[ ]:


recom_vali.columns = ["user_id","item_id","sim_VMSKNN"]

recom_vali["contadorFinalVMSKNN_Sim"] = 1

recom_vali['contadorFinalVMSKNN_Sim'] = recom_vali.groupby('user_id')['contadorFinalVMSKNN_Sim'].transform(pd.Series.cumsum)


recom_vali = recom_vali[recom_vali["sim_VMSKNN"]>0]
recom_vali.to_csv("../../user_data/tablas_boosting/VMSKNN_Train_"+str(NUM_RECOM)+".csv", index=False)
...
#recom_item = []                         # Guardaremos aquí, para cada usuario, su recomendación
recom_item = pd.DataFrame()

for c in range(now_phase + 1):
    print("phase "+ str(c))
    print("-----------------------------------------------------------")
    
    # Cargamos la tabla de train y la de test. Luego las juntamos y retiramos duplicados: 
    click_train = pd.read_csv(train_path + '/underexpose_train_click-{}.csv'.format(c),
                              header=None, names=['user_id', 'item_id', 'time']).sort_values(["user_id","time"])
    click_test = pd.read_csv(test_path + '/underexpose_test_click-{}/underexpose_test_click-{}.csv'.format(c,c),
                             header=None, names=['user_id', 'item_id', 'time']).sort_values(["user_id","time"])
    ...
    # Creamos la clase del modelo, CAMBIANDO, EN PRNCIPIO, SOLO LOS DOS PRIMEROS PARÁMETROS. Luego, lo entrenamos:  
    ...
    # Recomendación --> predict_next, de argumentos user_id, último click del user_id, listado de todos los objetos a recomendar. 

    ultimosclicks = all_click.groupby('user_id').tail(1)                                  #Juntamos últimos clicks de cada usuario 
    ultimosclicks = ultimosclicks[ultimosclicks["user_id"].isin(click_test["user_id"])]   # nos quedamos solo usuarios de test
    ultimosclicks = ultimosclicks[['user_id','item_id']]                                  # Las dos columnas relevantes

    objetos = all_click.item_id.unique()    #  Creamos un array con el listado de todos los items distintos que hay 

    for users in tqdm(click_test['user_id'].unique()):  # Para cada usuario de test, usamos la función, que devuelve una serie  
            lastclick =  int(ultimosclicks['item_id'][ultimosclicks['user_id']==users].values)
            preds = model.predict_next(users, lastclick, objetos).sort_values(ascending=False)  # lista con el item y su peso
        
            historial = all_click[all_click['user_id']==users].item_id.unique()         # todos los items clicados por el usuario
            preds_index = preds.index
            preds_index_filter = preds_index[~preds_index.isin(historial)]         # Eliminamos de la recomendacion los ya clickados
            for j in preds_index_filter[0:NUM_RECOM]: 
                recom_item_valid.append([users, j, preds[j]])
     
    recom_fase = pd.DataFrame(recom_item_valid, columns=['user_id', 'item_id', 'sim_VMSKNN']) # Hacemos df la reco de la fase
    recom_fase = recom_fase[recom_fase["sim_VMSKNN"]>0] # Eliminamos items de similaridad 0 (aleatorios)
    
    recom_item = recom_item.append(recom_fase) # Vamos juntando las recomendaciones de cada fase
...
recom_df = pd.DataFrame(recom_item, columns=['user_id', 'item_id', 'sim_VMSKNN']) 
recom_df["contadorFinalVMSKNN_Sim"] = 1
recom_df['contadorFinalVMSKNN_Sim'] = recom_df.groupby('user_id')['contadorFinalVMSKNN_Sim'].transform(pd.Series.cumsum)
recom_df = recom_df[recom_df["sim_VMSKNN"]>0]
recom_df.to_csv("../../user_data/tablas_boosting/VMSKNN_Score_"+str(NUM_RECOM)+".csv", index=False)
recom_df
